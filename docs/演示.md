...
2. 利用大语言模型对采集到的数据进行分析与归纳。
3. 生成包含靶点概述、在研管线、近期研究动态和竞争格局判断的结构化报告。
...
## 数据分析层设计
### 缓存设计
- 标准化缓存：按试验/文献外键复用最近的标准化结果。
- LLM 增强缓存：按试验/文献外键查找最近增强结果，避免同一试验或文献反复调用模型。
...
- 报告层不直接读取原始记录，而是从持久化的分析快照构造章节上下文。
- 四个章节分别使用不同的事实依据/上下文，并根据不同的提示词，独立生成：靶点概述、在研管线概览、近期研究动态、竞争格局判断。
- `ReportContextBuilder` 只把当前章节需要的内容、事实、全局信息传给报告生成器，以裁剪无效上下文。
...
- 分析聚合层：`AnalysisReadyBundle`、`SectionInputBundle` 按报告章节聚合组织事实、证据和覆盖情况。
...
1. 对来源数据检索条件、规范化筛选的规则、融合打分权重、裁剪规则进行参数微调，设置为可配置化。
2. 来源数据改为离线全量存储与增量更新，增强检索稳定性与分析可用性；记录级LLM增强的数据集改为离线构建，可显著减少执行在线任务时的等待时间。或者将在线检索的IO任务改为异步任务。
3. 增强优化缓存功能，在报告层引入更细粒度的缓存，例如章节级缓存或 Prompt 结果缓存。
4. 增加靶点信息数据源(UniProt)，增强最终报告中对靶点概览的介绍。
5. 增加 LLM 层的审计功能，引入 LangSmith，以便后续分析优化。
6. 存储层迁移到PostgresSQL。
7. 增强对领域模型的利用，提取更有价值的规范化信息。
