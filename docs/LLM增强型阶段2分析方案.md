...
- 对每条 `NormalizedTrialRecord` 和 `NormalizedLiteratureRecord` 做全量逐条 LLM 结构化增强
...
2. 持久化标准化记录
3. 对全部标准化试验逐条运行 `TrialLLMEnrichmentService`
4. 对全部标准化文献逐条运行 `LiteratureLLMEnrichmentService`
5. 生成 `rule score`
6. 解析 LLM 返回的 `llm score`
7. 计算 `final blended score`
8. 用 `final blended score` 更新 selector 和章节输入
