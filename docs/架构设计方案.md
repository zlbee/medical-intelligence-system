...
- `analysis_llm_enrichment_full_scan=false`
  - 仅对规则分 Top N 的 trial 和 literature 执行增强
- `analysis_llm_enrichment_top_n`
  - 控制 Top N 的规模，默认值为 `20`

这正是针对“数据源基数大、LLM 多次调用成本高”的核心控制开关。
...
`来源侧过滤与抓取上限 -> 标准化去重 -> 规则打分 -> Top N LLM 增强 -> 章节级证据聚合 -> 分章节生成`
...
| LLM 多次调用成本高、耗时长 | 报告生成慢，模型调用费用高 | 复用标准化缓存、复用 LLM 增强缓存、持久化分析快照和报告，必要时只对 Top N 记录做 LLM 增强 |
...
1. 将长耗时的分析和报告生成拆为异步任务，前端通过任务状态轮询或推送查看进度。
