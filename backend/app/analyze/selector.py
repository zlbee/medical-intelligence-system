...
from app.analyze.scoring import (
    score_literature_for_competition,
    score_literature_for_research_update,
    score_literature_for_target_overview,
    score_trial_for_competition,
    score_trial_for_pipeline,
)
...
class SectionSelector:
    ...
    def select_target_overview(
        ...
        ranked_literature = sorted(
            literature,
            key=lambda item: (
                score_literature_for_target_overview(query, item),
        ...
        ranked_trials = sorted(
            trials,
            key=lambda item: score_trial_for_pipeline(query, item),
            reverse=True,
        )
        ...
        selection = SectionSelection(
            ...
            selection_notes=[
                "Prefer review-style and high target-match literature for mechanism and context overview.",
    ...
    def select_pipeline_overview(
        ...
        selection = SectionSelection(
            trial_keys=[trial.trial_key for trial in selected_trials],
            selection_notes=[
                "Rank trials by target relevance, phase, active status, and whether results are available.",
            ],
        )
        if len(ranked_trials) > len(selected_trials):
            selection.truncation_notes.append(
    ...
    def select_research_update(
        ...
        recent_ranked = sorted(
            ...
            key=lambda item: (
                self._publication_ordinal(item),
                score_literature_for_research_update(query, item),
            ),
            reverse=True,
        )
        high_value_ranked = sorted(
            literature,
            key=lambda item: score_literature_for_research_update(query, item),
            reverse=True,
        )
    ...
    def select_competition_assessment(
        ...
        ranked_trials = sorted(
            trials,
            key=lambda item: score_trial_for_competition(query, item),
            reverse=True,
        )
        ranked_literature = sorted(
            literature,
            key=lambda item: score_literature_for_competition(query, item),
            reverse=True,
        )
        ...
        selection = SectionSelection(
            ...
            literature_keys=[paper.literature_key for paper in selected_literature],
            selection_notes=[
                "Prefer trials that are active, late-stage, sponsor-identifiable, and relevant to the target.",
                "Keep literature with strong target match or explicit NCT mentions as supporting competition evidence.",
            ],
        )
        if len(ranked_trials) > len(selected_trials):
            selection.truncation_notes.append(
